...
class UnitCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      subtitle: unit.description.isNullOrEmpty
          ? null
          : Text(unit.description ?? '', style: theme.textRegular14Subtle),
      onTap: onTap,
      tileColor: color,
    );
  }
    ...
    return Stack(
      children: [
        UnitCard(
          unit: unit,
          color: index.color,
          onTap: canEdit ? () => AddUnit(unit: unit).show(context) : null,
        ),
        Positioned(
          top: 8,
          right: 8,
          child: Consumer(
            builder: (BuildContext context, WidgetRef ref, Widget? child) {
              final enable = ref.watch(
                  multiSelectUnitProvider.select((value) => value.enable));
              ...
              return Consumer(
                builder: (BuildContext context, WidgetRef ref, Widget? child) {
                  final isSelected = ref.watch(multiSelectUnitProvider
                      .select((value) => value.data.contains(unit)));
                  return Checkbox(
                    value: isSelected,
                    onChanged: (value) {
